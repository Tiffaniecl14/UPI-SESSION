...
def euler_explicite(t0,h,N, y0,f):
    ...
    #definie les differentes subdivisions ou on va etudier la solution approchée
    t = np.linspace(t0,t0+N*h,N+1) # N+1 valeurs entre t0 et t0+N*h

    # Alloue un tableau de la taille de t, initialisé à 0. Il servira à
    # stocker la solution aux instants t0+i*h.
    y = np.zeros((N+1)) #creation d'un vecteur de taille n+1

    # Donnée initiale, la valeur [0] du tableau y
    y[0] = y0

    # On doit faire N itérations en temps
    #on calcul la solution approchée au point de subdivision choisi
